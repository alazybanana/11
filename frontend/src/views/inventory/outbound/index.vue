...
import { listLocations, listTransactions, listWarehouses, stockDecrease } from '@/api/inventory'
...
const { loading, rows, total, page, pageSize, query, load, search, reset, changePage, changeSize } =
  usePagedTable<Transaction, { keyword: string; warehouse_id: number | undefined; date_from: string; date_to: string }>(
    (params) => listTransactions({ ...params, transaction_type: 'OUT' }),
...
async function submit(): Promise<void> {
  ...
  try {
    const result = await stockDecrease({
    ...
    ElMessage.success(`出库成功，流水号 ${result.transaction_no}，结存 ${result.quantity_after}`)
    form.quantity = 1
    form.remark = ''
    await load()
  } catch (error) {
...
<template>
  <div>
    <el-card shadow="never">
      <template #header>
        <div class="table-toolbar">
          <span class="page-title">手工出库</span>
        </div>
      </template>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
        <el-row :gutter="12">
          ...
          <el-col :span="16">
            <el-form-item label="备注">
              <el-input v-model="form.remark" placeholder="出库原因 / 说明" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item>
        ...
        <div class="form-tip">库存不足时后端会拒绝出库（错误码 5001），本次操作整体回滚。</div>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <template #header>
        <div class="table-toolbar">
          <span class="page-title">出库流水</span>
        </div>
      </template>
      ...
      <el-table v-loading="loading" :data="rows" border size="small">
        ...
        <el-table-column label="出库数量" prop="quantity_change" width="110" align="right" />
        ...
        <template #empty>暂无出库流水</template>
      </el-table>
